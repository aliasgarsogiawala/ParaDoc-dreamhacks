...
const symptomCategories = [
  { name: "Neurological", icon: <Brain size={18} />, examples: "Headache, Dizziness, Confusion" },
  { name: "Cardiovascular", icon: <Activity size={18} />, examples: "Chest pain, Palpitations, Fatigue" },
  { name: "Fever & Pain", icon: <Thermometer size={18} />, examples: "Fever, Body aches, Chills" },
  { name: "Medication", icon: <Pill size={18} />, examples: "Side effects, Interactions, Dosage" },
];
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-rose-50 to-white dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-500 overflow-hidden">
            ...
            >
              ...
              <div className="relative bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-lg p-4 border border-rose-100 dark:border-slate-700 flex items-center">
                <div className="bg-rose-100 dark:bg-rose-900/30 p-2 rounded-full mr-3">
                  <Heart size={18} className="text-rose-500 dark:text-rose-300" />
                </div>
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-rose-600 dark:text-rose-300">Daily Health Tip</h3>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {symptomCategories.map((category, index) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-500 dark:text-gray-400 mt-1 text-center line-clamp-1">{category.examples}</span>
                      </button>
                    ))}
                  </div>
                </div>
                    ...
                    fallback={
                      <div className="h-[300px] flex items-center justify-center">
                        <div className="flex flex-col items-center">
                          ...
                          <p className="mt-4 text-rose-500 dark:text-rose-300 font-medium">Preparing your consultation...</p>
                        </div>
                      </div>
                    }
                  >
                  ...
                  <ul className="space-y-2">
                    {emergencyWarnings.map((warning, index) => (
                      <li key={index} className="flex items-start">
                        ...
                        <span className="text-sm text-red-600 dark:text-red-200">{warning}</span>
                      </li>
                    ))}
                  </ul>
                  <div className="mt-4 pt-3 border-t border-red-200 dark:border-red-800/30">
                    <p className="text-sm text-red-600 dark:text-red-300 font-medium">
                      In case of emergency, call your local emergency number (101 in India) immediately.
                    </p>
                  </div>
                </div>
              </motion.div>
                ...
                <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-md p-6 border border-rose-100 dark:border-slate-700">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {healthResources.map((resource, index) => (
                      ...
                      >
                        ...
                        <ArrowRight size={16} className="text-rose-400 dark:text-rose-300 opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
                      </Link>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-rose-100 dark:border-slate-700">
                    <p className="text-xs text-gray-500 dark:text-gray-400 italic">
                      Disclaimer: The information provided is for general informational purposes only and should not replace professional medical advice.
                    </p>
                  </div>
                </div>
              </motion.div>
